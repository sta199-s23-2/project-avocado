...
lm_model <- lm(formula = candpct ~ candnumber + quality_cand + qualnumber + 
    openseat + dist_income + total_primary, data = candidate_final)

ggPredict(lm_model,colorAsFactor = TRUE,interactive=TRUE)

```

question: *Since ggPredict only takes 3 variables, how should we visualize our interaction model?*
