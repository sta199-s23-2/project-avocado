...
The correlation between percentage of residents with four-year college degrees and the median income is linear, positive, and ver strong, which makes real-world sense. 
```{r}
#| label: EDA2
...
**Primary Barriers to Working Class Representation** ^[Treul, S. A., & Hansen, E. R. (2023). Primary Barriers to Working Class Representation. Political Research Quarterly, 0(0). https://doi.org/10.1177/10659129231154914]
...
We end up choosing the model we get from backward selection over the model from the forward selection because the backward selection model yields a lower AIC, indicating that the model is better.
The variables includes in the model are candnumber, quality_cand, qualnumber, openseat, dist_income, total_primary
...
AIC for interaction model: -447.1903
Since the AIC for the interaction model is much larger than the additive model we get from the backward selection, we will stick to the additive model.
